...
static BDCallRecord *_BDCallRecords;
//static int otp_record_num;
//static int otp_record_alloc;
static int _BDRecordNum;
static int _BDRecordAlloc;
...
static inline uintptr_t pop_call_record() {
    ...
    if (cs->is_main_thread && _call_record_enabled) {
        ...
        if (cost > _min_time_cost && cs->index < _max_call_depth) {
            if (!_BDCallRecords) {
                _BDRecordAlloc = 1024;
                _BDCallRecords = malloc(sizeof(BDCallRecord) * _BDRecordAlloc);
            }
            _BDRecordNum++;
            if (_BDRecordNum >= _BDRecordAlloc) {
                _BDRecordAlloc += 1024;
                _BDCallRecords = realloc(_BDCallRecords, sizeof(BDCallRecord) * _BDRecordAlloc);
            }
            BDCallRecord *log = &_BDCallRecords[_BDRecordNum - 1];
            log->cls = pRecord->cls;
            log->depth = curIndex;
            log->sel = pRecord->cmd;
            log->time = cost;
...
void BDCallTraceStop() {
    _call_record_enabled = false;
}

void BDCallConfigMinTime(uint64_t us) {
    _min_time_cost = us;
}
void BDCallConfigMaxDepth(int depth) {
    _max_call_depth = depth;
}

BDCallRecord *BDGetCallRecords(int *num) {
    if (num) {
        *num = _BDRecordNum;
    }
    return _BDCallRecords;
}

void BDClearCallRecords() {
    if (_BDCallRecords) {
        free(_BDCallRecords);
        _BDCallRecords = NULL;
    }

...
@implementation BDLagDB

+ (BDLagDB *)shareInstance {
    static BDLagDB *instance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[BDLagDB alloc] init];
    });
    return instance;
}
...
- (RACSignal *)increaseWithStackModel:(BDCallStackModel *)model {
    ...
    return [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
        if ([model.stackStr containsString:@"+[BDCallStack callStackWithType:]"] || [model.stackStr containsString:@"-[BDLagMonitor updateCPUInfo]"]) {
            return nil;
        }
        @strongify(self);
...
- (RACSignal *)selectClsCallWithPage:(NSUInteger)page {
    ...
    return [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
        ...
        if ([db open]) {
            ...
            NSMutableArray *arr = [NSMutableArray array];
            while ([rs next]) {
                BDCallTraceTimeCostModel *model = [self clsCallModelFromResultSet:rs];
                [arr addObject:model];
                count ++;
            }
            if (count > 0) {
...
//结果封装成 model
- (BDCallTraceTimeCostModel *)clsCallModelFromResultSet:(FMResultSet *)rs {
    BDCallTraceTimeCostModel *model = [[BDCallTraceTimeCostModel alloc] init];
